use super::{DummyDataLoader,sign_tx_sha256_by_input_group,sign_tx_sha256,sign_tx_sha256_without_pubkey,ripemd160,sha256,pubkey_compressed,pubkey_uncompressed,ripemd_sha,SigType, RIPEMD160_SHA256_ALL_BIN, MAX_CYCLES, SECP256K1_DATA_BIN};
...
fn test_sighash_all_unlock() {
    ...
    let tx = sign_tx_sha256(tx, &secret_key);
...
fn test_sighash_all_unlock_without_pubkey() {
    ...
    let tx = sign_tx_sha256_without_pubkey(tx, &secret_key);
    let resolved_tx = build_resolved_tx(&data_loader, &tx);
    let verify_result =
        TransactionScriptsVerifier::new(&resolved_tx, &data_loader).verify(MAX_CYCLES);
    verify_result.expect("pass verification");
...
fn test_sighash_all_unlock_with_uncompressed_pubkey_and_non_recoverable_signature() {
    ...
    let tx = sign_tx_sha256_by_input_group(tx, &secret_key, 0, 0, SigType::NonRecoverable);
    let resolved_tx = build_resolved_tx(&data_loader, &tx);
    let verify_result =
        TransactionScriptsVerifier::new(&resolved_tx, &data_loader).verify(MAX_CYCLES);
    verify_result.expect("pass verification");
...
fn test_signing_with_wrong_key() {
    ...
    let tx = sign_tx_sha256(tx, &wrong_secret_key);
    let resolved_tx = build_resolved_tx(&data_loader, &tx);
    let verify_result =
        TransactionScriptsVerifier::new(&resolved_tx, &data_loader).verify(MAX_CYCLES);
    assert_error_eq!(
...
fn test_signing_with_wrong_key_without_pubkey() {
    ...
    let tx = sign_tx_sha256_without_pubkey(tx, &wrong_secret_key);
    let resolved_tx = build_resolved_tx(&data_loader, &tx);
    let verify_result =
        TransactionScriptsVerifier::new(&resolved_tx, &data_loader).verify(MAX_CYCLES);
    assert_error_eq!(
...
fn test_wrong_size_witness_args() {
    ...
    let tx = sign_tx_sha256(raw_tx.clone(), &secret_key);
