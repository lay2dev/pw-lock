...
int hash_address(mol_seg_t *script_seg, unsigned char * hash){
  ...
  /* the type id of secp256k1_blake160_signhash_all: 0x9bd7e06f3ecf4be0f2fcd2188b23f1b9fcc88e5d4b65a8637b17723bbda3cce8 */
  unsigned char SECP256K1_BLAKE160_SIGHASH_ALL_TYPE_HASH[32] = {
      0x9b, 0xd7, 0xe0, 0x6f, 0x3e, 0xcf, 0x4b, 0xe0, 0xf2, 0xfc, 0xd2, 0x18, 0x8b, 
      0x23, 0xf1, 0xb9, 0xfc, 0xc8, 0x8e, 0x5d, 0x4b, 0x65, 0xa8, 0x63, 0x7b, 0x17, 
      0x72, 0x3b, 0xbd, 0xa3, 0xcc, 0xe8
  };
  /* the type id of secp256k1_blake160_multisig_all: 0x5c5069eb0857efc65e1bca0c07df34c31663b3622fd3876c876320fc9634e2a8 */
  unsigned char SECP256K1_BLAKE160_MULTISIG_ALL_TYPE_HASH[32] = {
      0x5c, 0x50, 0x69, 0xeb, 0x08, 0x57, 0xef, 0xc6, 0x5e, 0x1b, 0xca, 0x0c, 0x07,
      0xdf, 0x34, 0xc3, 0x16, 0x63, 0xb3, 0x62, 0x2f, 0xd3, 0x87, 0x6c, 0x87, 0x63,
      0x20, 0xfc, 0x96, 0x34, 0xe2, 0xa8
  };
  ...
  if (code_hash_seg.size == 0) {
    ...
    if (memcmp(code_hash_seg.ptr, SECP256K1_BLAKE160_SIGHASH_ALL_TYPE_HASH, code_hash_seg.size) == 0) {
      /* generate short ckb address */
      payload[payload_len++] = 0x01;
      payload[payload_len++] = 0x00;
    } else if(memcmp(code_hash_seg.ptr, SECP256K1_BLAKE160_MULTISIG_ALL_TYPE_HASH, code_hash_seg.size) == 0){
      /* generate short ckb address */
      payload[payload_len++] = 0x01;
      payload[payload_len++] = 0x01;
    } else {
    ...
    if(ret == 0) return -10;
    ret = bech32_encode((char *)&ckb_address, CKB_ADDRESS_PREFIX, data, data_len);
    if(ret == 0) return -11;
    data_len += 10;

    /* shorten ckb address to 17 characters, using ... to substitute the middle characters */
    if (data_len <= 17) {
...
/*
* Format amount to number with 8 decimals and ended with "CKB", get the keccak256 hash of the amount string
* 
* Arguments:
...
int calculate_typed_data(unsigned char *tx_message, unsigned char * typed_data_hash){
  ...
  /* hash for type CKBTransaction, is equal to web3utils.sha3('CKBTransaction(bytes32 hash,string fee,string input-sum,Output[] to)Output(string address,string amount)') */
  ...
  /* hash for type Output, is equal to web3utils.sha3('Output(string address,string amount)') */
  ...
  /* hash for domain separator, is equal to web3utils.sha3("EIP712Domain(string name,string version,uint256 chainId,address verifyingContract)") */
   ...
   * we set the tx fee to zero. The rules for dao withdraw tx should be followed, 
   * when build typed data at other places.
   */
...
/*
 * Verify the transaction using secp256k1 as sig algorithm and keccak256 as hash algorithm.
 * 
 * Since not all ethereum wallets support EIP712, the verification will support two hashes 
 * for transaction, both of them are ok.
 * 1. ethereum peronsal hash
 * 2. EIP712 typed data hash
 * 
 * Arguments:
 * eth_address, keccak256 hash of pubkey last 20 bytes, used to shield the real pubkey.
 *
 * Witness:
 * WitnessArgs with a signature in lock field used to present ownership.
 */
int verify_secp256k1_keccak_sighash_all(unsigned char eth_address[BLAKE160_SIZE]) {
  int ret;
  uint64_t len = 0;
  unsigned char temp[TEMP_SIZE];
