...
int read_args(unsigned char *pubkey_hash, uint64_t *min_ckb_amount,
  ...
  if (args_bytes_seg.size < RIPEMD160_SIZE ||
      args_bytes_seg.size > RIPEMD160_SIZE + 2) {
    return ERROR_ARGUMENTS_LEN;
  }
  memcpy(pubkey_hash, args_bytes_seg.ptr, RIPEMD160_SIZE);
  *min_ckb_amount = 0;
  *min_udt_amount = 0;
  if (args_bytes_seg.size > RIPEMD160_SIZE) {
    int x = args_bytes_seg.ptr[RIPEMD160_SIZE];
    int is_overflow = quick_pow10(x, min_ckb_amount);
    if (is_overflow) {
      *min_ckb_amount = MAX_UINT64;
    }
  }
  if (args_bytes_seg.size > RIPEMD160_SIZE + 1) {
    int x = args_bytes_seg.ptr[RIPEMD160_SIZE + 1];
...
int main() {
  int ret;
  int has_sig;
  unsigned char pubkey_hash[RIPEMD160_SIZE];
  ...
  if (has_sig) {
    ...
    return verify_secp256k1_ripemd160_sha256_sighash_all(pubkey_hash);
  } else {
    /* unlock via payment */
